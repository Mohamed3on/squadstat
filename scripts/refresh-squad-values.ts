...
/** TM serves 25 teams a page, and both tables are four pages deep. A short page
...
/** All four pages of one table, each checked for a full page. */
async function scrape<T extends { id: string }>(
  ...
  const pages = await Promise.all(
    PAGES.map(async (page) => {
      const rows = parse(await fetchPage(url(page)));
      if (rows.length !== PER_PAGE) {
        throw new Error(
          `Parsed ${rows.length} ${what} from page ${page} (expected ${PER_PAGE}) — selectors moved or Transfermarkt is rate limiting.`,
        );
      }
      return rows;
    }),
  ...
  if (ids.size !== rows.length) {
    throw new Error(
      `${rows.length - ids.size} ${what} appear twice across the four pages — the table moved mid-scrape.`,
    );
  }
  return rows;
}
...
async function main() {
  const [clubs, teams] = await Promise.all([
    scrape("clubs", clubsUrl, parseClubs),
